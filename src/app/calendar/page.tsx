  ...
  return (
    <>
      <PageHeader
        ...
        deck="This is the local calendar the way we've actually raced it. The Honolulu Marathon sits at the end of the year and the whole running calendar leans toward it, and the tris bunch up in late spring and fall. Dates move around, so check with the organizer before you build a season around one."
      >
          ...
          {byMonth.map((m, i) => (
            <li key={m.name} id={m.name.toLowerCase()} className="grid gap-4 md:grid-cols-[10rem_1fr] py-8">
              <div>
                ...
                {i === 8 ? <p className="mt-2 text-sm text-ink-soft">This is the hottest stretch of the year, so everything below is a heat race whether it says so or not.</p> : null}
              </div>
